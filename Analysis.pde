...
String[] names = {"Reference", "You (1st Alto)", "You (1st Alto, 2nd take)", "You (1st Alto, 3rd Take)"};
String[] files = {"SA-MB-09-002.wav", "SA-XB-03-002.wav", "SA-XB-03-001.wav", "SA-XB-03-003.wav"};
...
int currentMeasure;
int numberOfMeasuresVisualized;
int lastMeasure;
float[] keySignature = {55, 58, 62, 65, 69};  //key for leger lines (piece specific)
...
void setup() {
  ...
  zoomInButton = new ZoomButton(width-140, height-50, 30, "+");
  zoomOutButton = new ZoomButton(width-175, height-50, 30, "-");
  ...
  visualizationButtons[1] = new VisualizationButton("Timings", visualizationButtonCorners[1], visualizationButtonCorners[3], visualizationButtonDimensions[0], visualizationButtonDimensions[1]);
...
void draw() {
  ...
  if (numberOfMeasuresVisualized > 1) {
    text("Basie-Straight Ahead :: Measure " + (currentMeasure+1) + "–" + (currentMeasure+numberOfMeasuresVisualized) + " of " + (lastMeasure+1), width-450, 50);
  } else {
    text("Basie-Straight Ahead :: Measure " + (currentMeasure+1) + " of " + (lastMeasure+1), width-450, 50);
  }
  for (int i = 0; i < performances.length; i++) {
  ...
  rightButton.display();
  leftButton.display();
  zoomOutButton.display(currentMeasure+numberOfMeasuresVisualized <= lastMeasure);
  zoomInButton.display(numberOfMeasuresVisualized > 1);
  moveView();
...
void keyPressed() {
  ...
  if (key == 'h') {
    currentMeasure = 0;
    numberOfMeasuresVisualized = lastMeasure;
    setNewStartEnd();
  }
}
...
void mousePressed() {
  ...
  if (zoomOutButton.pressed()) {
    numberOfMeasuresVisualized = constrain(numberOfMeasuresVisualized+1,1,lastMeasure-currentMeasure+1);
    setNewStartEnd();
  }
